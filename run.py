...
async def gather_all_providers() -> list[ProviderResult]:
...
async def main() -> None:
    ...
    while attempt <= BRIEF_MAX_ATTEMPTS:
        results = await gather_all_providers()

        for r in results:
            await _persist(r, brief_id, date_val)
    ...
    # 6. LLM — now returns 5 fields
    narrative, headline, headline_summary, daily_title, daily_text, src = generate(ctx, status)
    logger.info("Narrative source: %s", src)

    # 7. Render
    html = render_html(
        narrative, ctx, status,
        headline=headline,
        headline_summary=headline_summary,
        daily_summary_title=daily_title,
        daily_summary_text=daily_text,
    )
