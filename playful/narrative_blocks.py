...
Per-block narratives живут ТОЛЬКО в briefs.narrative_blocks (jsonb). В Telegram
...
{"text": "2-4 предложения, до 500 символов"}
...
weather: {"text": "Утро ясное, днём до +24°, к вечеру прохладнее на 4°. Ветер слабый, окно для дел на улице — с 9 до 17, дальше поднимать не стоит. Если план был «дойти пешком до встречи» — самое время."}
tasks: {"text": "На сегодня 39 задач, из них 2 на p3 («Оформить пропуск в БЦ», «Подготовить ТЗ по CRM»). Всё остальное — p4–p5, оно подождёт. Начни с пропуска: мелкая, но открывает доступ к БЦ, без неё CRM-кейс не сдвинуть."}
movement: {"text": "Вчера 4348 шагов — меньше 7к, к балансу −287 ккал (съел 2104, потратил 2391). Сегодня окно для компенсации: до обеда дойти до 5к, вечером ещё 3к. Не геройствуй — две обычные прогулки, не спортивная сессия."}
calendar: {"text": "Одна встреча в обед, остальное время — твоё. Утром — Deep Work без переключений, после обеда — лёгкие задачи и движение. День для главного."}
battery: {"text": "97 батарейка после 5ч 58м сна — это не логика, это ты. HRV 70, стресс 11 — тело в полном порядке. Используй это до обеда: главная задача, Deep Work, никакого шума. После 15:00 ресурс просядет, планируй откат."}"""
...
def _format_user_prompt(block_name: str, facts: dict[str, Any]) -> str:
    ...
    lines.append("")
    lines.append("Сгенерируй JSON с одним полем text (2-4 предложения, до 500 символов).")
    return "\n".join(lines)
...
def _fallback_block_text(block_name: str, facts: dict[str, Any]) -> str | None:
    ...
    if block_name == "calendar":
        ...
        if free:
            return "Сегодня нет встреч — свободный день. Потрать его на главную задачу и движение."
    ...
    if block_name == "battery":
        ...
        parts.append("Бери главное до обеда, потом просядет.")
        return " ".join(parts)[:MAX_BLOCK_CHARS]
...
async def _compose_block_narrative_async(
    ...
    *,
    ...
    try:
        ...
        logger.warning("block-narrative[%s]: JSON parse failed: %s; raw=%r",
                       block_name, e, raw[:200])
        fb = _fallback_block_text(block_name, facts)
        return (fb or "", "fallback" if fb else "empty", "json-parse")

    if not isinstance(data, dict) or "text" not in data:
        logger.warning("block-narrative[%s]: missing 'text' key, raw=%r", block_name, raw[:200])
        fb = _fallback_block_text(block_name, facts)
        return (fb or "", "fallback" if fb else "empty", "missing-key")
